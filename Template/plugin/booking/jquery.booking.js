...
(function ($, doc, win) {
    ...
    var booking = function (object, option) {
        ...
        this.build = function () {
            ...
            $this
                .find(".template-component-booking-vehicle-list")
            ...
            $this
                .find(
                    '.template-component-booking-package-list>li>.template-component-button-box>a[href="#"]'
                )
                .on("click", function (e) {
                    e.preventDefault();
            ...
            $this
                .find(
                    ".template-component-booking-service-list>li>.template-component-button-box>a"
                )
                .on("click", function (e) {
                    e.preventDefault();
        ...
        this.vehicleSelect = function (object) {
            var vehicleList = $this.find(
                ".template-component-booking-vehicle-list>li"
            );

            if (typeof object === typeof undefined)
                object = vehicleList.first();
        ...
        this.vehicleGet = function () {
            var selected = $this.find(
                ".template-component-booking-vehicle-list>li.template-state-selected"
            );
        ...
        this.packageSelect = function (object) {
            var packageList = $this.find(
                ".template-component-booking-package-list>li"
            );

            if (typeof object === typeof undefined)
                object = packageList.first(
                    ':not[class="template-state-hidden"]'
                );
        ...
        this.packageSet = function () {
            ...
            packageList.removeClass(
                "template-state-hidden template-state-selected"
            );
            ...
            packageList.each(function () {
                var vehicleIdRelated = $self.getAttributeValue(
                    $(this),
                    "data-id-vehicle-rel"
                );
            ...
            packageList.each(function () {
                if (!$(this).hasClass("template-state-hidden")) {
                    if (
                        $(this)
                            .find(".template-component-button")
                            .attr("href") === "#"
                    ) {
        ...
        this.serviceSet = function () {
            var serviceList = $this.find(
                ".template-component-booking-service-list>li"
            );
            serviceList.removeClass(
                "template-state-hidden template-state-selected"
            );
            ...
            if (packageId !== false) {
                $this
                    .find(
                        '.template-component-booking-package-list>li[data-id="' +
                            packageId[0] +
                            '"]>.template-component-booking-package-service-list>li'
                    )
                    .each(function () {
                        var packageServiceListId = $self.getAttributeValue(
                            $(this)
                        );
                        if (packageServiceListId !== false) {
                            serviceList.each(function () {
                                var serviceId = $self.getAttributeValue(
                                    $(this)
                                );
                ...
                serviceList.each(function () {
                    var packageIdRelated = $self.getAttributeValue(
                        $(this),
                        "data-id-package-rel"
                    );
        ...
        this.calculate = function () {
            ...
            if (packageSelected.length === 1) {
                cost +=
                    parseFloat(
                        packageSelected
                            .find(
                                ".template-component-booking-package-price-total"
                            )
                            .text()
                    ) +
                    parseFloat(
                        "0." +
                            packageSelected
                                .find(
                                    ".template-component-booking-package-price-decimal"
                                )
                                .text()
                    );
                duration += parseInt(
                    packageSelected
                        .find(
                            ".template-component-booking-package-duration-value"
                        )
                        .text(),
                    10
                );
            ...
            var serviceSelected = $this.find(
                ".template-component-booking-service-list>li.template-state-selected"
            );
            serviceSelected.each(function () {
                cost += parseFloat(
                    $(this)
                        .find(".template-component-booking-service-price-value")
                        .text()
                );
                duration += parseInt(
                    $(this)
                        .find(
                            ".template-component-booking-service-duration-value"
                        )
                        .text(),
                    10
                );
            ...
            $this
                .find(".template-component-booking-summary-price-value")
                .text(parseFloat(cost).toFixed(0));

            $this
                .find(
                    ".template-component-booking-summary-duration>h5>span:eq(0)"
                )
                .text(duration[0]);
            $this
                .find(
                    ".template-component-booking-summary-duration>h5>span:eq(2)"
                )
                .text(duration[1]);
        };
        ...
        this.setData = function () {
            ...
            var vehicleSelected = $this.find(
                ".template-component-booking-vehicle-list>li.template-state-selected"
            );
            data.vehicle.name = vehicleSelected
                .children("div")
                .children("div:eq(1)")
                .text();
            ...
            if (packageSelected.length === 1) {
                data.package.name = packageSelected
                    .find(".template-component-booking-package-name")
                    .text();
                data.package.price =
                    packageSelected
                    ...
                    packageSelected
                        ...
                        .text();
                data.package.currency = packageSelected
                    .find(".template-component-booking-package-price-currency")
                    .text();
                data.package.duration = packageSelected
                    .find(".template-component-booking-package-duration-value")
                    .text();

                packageSelected
                    .find(".template-component-booking-package-service-list>li")
                    .each(function () {
            ...
            serviceSelected.each(function () {
                data.service.push({
                    name: $(this)
                        .find(".template-component-booking-service-name>span")
                        .text(),
                    price: $(this)
                        .find(".template-component-booking-service-price-value")
                        .text(),
                    currency: $(this)
                        .find(
                            ".template-component-booking-service-price-currency"
                        )
                        .text(),
                    duration: $(this)
                        .find(
            ...
            data.cost.price.value = $this
                .find(".template-component-booking-summary-price-value")
                .text();
            data.cost.price.currency = $this
                .find(".template-component-booking-summary-price-symbol")
                .text();

            data.cost.duration.hour = $this
                .find(
                    ".template-component-booking-summary-duration>h5>span:eq(0)"
                )
                .text();
            data.cost.duration.minute = $this
                .find(
                    ".template-component-booking-summary-duration>h5>span:eq(2)"
                )
                .text();
                        ...
                        $("#" + response.info[key].fieldId)
                            ...
                            .qtip({
                                ...
                                style: {
                                    classes:
                                        response.error === 1
                                            ? "template-qtip template-qtip-error"
                                            : "template-qtip template-qtip-success",
                                },
                                content: {
        ...
        this.blockForm = function (block) {
            if (block)
                $this.block({ message: false, overlayCSS: { opacity: "0.3" } });
            else $this.unblock();
        };
        ...
        this.setWidthClass = function () {
            ...
            if (oldClassName !== false)
                $this.removeClass(classPrefix + oldClassName);
        ...
        this.getAttributeValue = function (object, attributeName) {
            if (typeof attributeName === typeof undefined)
